...
class LineProducingSpecies:
    ...
    def get_line_opacities_emissivities(self, device : torch.device=torch.device("cpu")) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        device_pops = self.population.get().to(device)
        device_einstein_A = self.linedata.A.get().to(device)
        device_einstein_Ba = self.linedata.Ba.get().to(device)
        device_einstein_Bs = self.linedata.Bs.get().to(device)
        device_irad = self.linedata.irad.get().to(device)
        device_jrad = self.linedata.jrad.get().to(device)
    ...
    def get_line_widths(self, device: torch.device = torch.device("cpu")) -> torch.Tensor:
        ...
        return self.linedata.frequency.get().to(device).reshape(1,-1)*torch.sqrt(self.linedata.inverse_mass.get()
               * (astropy.constants.k_B*2.0/astropy.constants.c**2/astropy.constants.u).value*self.dataCollection.get_data("gas temperature").get().to(device).reshape(-1,1)#type: ignore
                 + self.dataCollection.get_inferred_dataset("vturb2 normalized").get().to(device).reshape(-1,1))
        #output format ([parameters.npoints, linedata.nrad])
    ...
    def get_line_frequencies(self, device: torch.device = torch.device("cpu")) ->torch.Tensor:
        ...
        #point, line, quadidx
        temp = self.linedata.frequency.get().to(device)[None, :, None] + self.linequadrature.roots.get().to(device)[None, None, :] * linewidths_device[:,:, None]
    ...
    def get_relevant_line_indices(self, opacity_compute_frequencies: torch.Tensor, sorted_linewidths_device: torch.Tensor, sorted_linefreqs_device: torch.Tensor, device: torch.device = torch.device("cpu")) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Args:
            opacity_compute_frequencies (torch.Tensor): Given frequencies for each point. Has dimensions [parameters.npoints, NFREQS]
            sorted_linewidths_device (torch.Tensor): Sorted line widths of the lines (sorted for each point). Has dimensions [parameters.npoints, self.linedata.nrad]
    ...
    def evaluate_line_opacities_emissivities(self, frequencies: torch.Tensor, min_line_indices: torch.Tensor, max_line_indices: torch.Tensor, line_opacities: torch.Tensor, line_emissivities: torch.Tensor, sorted_linefreqs_device: torch.Tensor, sorted_linewidths_device: torch.Tensor, device: torch.device = torch.device("cpu")) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        extended_point_indices = torch.div(scatter_indices, Nfreqs).type_as(scatter_indices)
