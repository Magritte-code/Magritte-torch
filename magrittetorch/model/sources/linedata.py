...
class Linedata:
    def __init__(self, params: Parameters, dataCollection : DataCollection, lineproducingspeciesidx: int) -> None:
        ...
        self.Ba: StorageTensor = StorageTensor(Types.LevelPopsInfo, [self.nrad], units.m**2*units.J**-1*units.s**-1, storagedir+"Ba"); self.dataCollection.add_data(self.Ba, "Einstein Ba_"+str(lineproducingspeciesidx))#einstein Ba coefficients (absorption)
        self.Bs: StorageTensor = StorageTensor(Types.LevelPopsInfo, [self.nrad], units.m**2*units.J**-1*units.s**-1, storagedir+"Bs"); self.dataCollection.add_data(self.Bs, "Einstein Bs_"+str(lineproducingspeciesidx))#einstein Bs coefficients (stimulated emission)
