...
from all_constant_single_ray_no_saving_model import create_and_run_model as all_constant_no_save_setup_and_run
from all_zero_single_ray import create_and_run_model as all_zero_setup_and_run
# from all_zero_single_ray import run_model as all_zero_run
from density_distribution_1D import create_and_run_model as density_dist_setup_and_run
...
from constant_velocity_gradient_1D import create_and_run_model as velocity_gradient_setup_and_run
...
class TestInput:
    def test_read_nonexistent_file(self):
        ...
        with pytest.raises(Exception):
            ...
            model = Model(bogusFile) #reading bogus files should raise an exception
            model.read()
...
class TestAnalytic:
    ...
    class TestAllConstant:
        #testing whether the setup runs (and saves)
    ...
    class TestAllConstantNoSave:
        def test_all_constant_no_save_setup_and_run(self):
            assert all_constant_no_save_setup_and_run(nosave=True)

    class TestAllZero:
        #testing whether the setup runs
        def test_all_zero_setup_and_run(self):
            all_zero_setup_and_run(nosave=True)


#     @pytest.mark.incremental
    class TestDensityDistribution1D:
        def test_density_distribution1D_setup_and_run(self):
            density_dist_setup_and_run("a", nosave=True, use_widgets=False)
